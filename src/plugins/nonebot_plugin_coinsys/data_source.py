...
async def randpic(event: GroupMessageEvent):
    """
    随机抽取一张图片
    """
    pic_path = russian_path / "data" / "coinsys" / "pic" / str(event.group_id)
    # 判断是否存在这个文件夹
    # 判断路径下是否有文件
    if not os.path.isdir(pic_path) or not os.listdir(pic_path):
        logger.error(f"抽图片：未找到路径{pic_path}或图片，使用默认图库")
        pic_path = russian_path / "data" / "coinsys" / "pic" 
    # 随机选取一个文件
    file_list = [file for file in os.listdir(pic_path) if os.path.isfile(os.path.join(pic_path, file))]
    file_name = random.choice(file_list)
    logger.info(f"抽到的图片为{pic_path / file_name}")
    return MessageSegment.image(pic_path / file_name)
...
async def rank(player_data: dict, group_id: int, type_: str) -> str:
    ...
    if type_ == "gold_rank":
        ...
        rank_name = "\t花费金币排行榜\n"
        all_user_data = [player_data[group_id][x]["cost_gold"] for x in all_user]
    rst = ""
    if all_user:
...
class RussianManager:
    def __init__(self):
        ...
        file = russian_path / "data" / "coinsys" / "russian_data.json"
        self.file = file
    ...
    def cost(self, gold: int, event: GroupMessageEvent) -> Tuple[str, int]:
        """
        消费
        :param event: event
        """
        self._init_player_data(event)
        if self._player_data[str(event.group_id)][str(event.user_id)]["gold"] < gold:
            return "你的金币不足以进行本次消费.", -1
        self._player_data[str(event.group_id)][str(event.user_id)]["gold"] -= gold
        self._player_data[str(event.group_id)][str(event.user_id)]["cost_gold"] += gold
        self.save()
        return (
            random.choice([f"欢迎下次光临...", "老板大气."]) + f"\n你消费了 {gold} 金币",
            gold,
        )
    ...
    async def rank(self, msg: str, group_id: int) -> str:
        ...
        if msg in ["花费排行", "消费排行"]:
            return await rank(self._player_data, group_id, "cost_gold")
    ...
    def _init_player_data2(self, user_id:str, group_id:str, nickname:str):
        """
        处理用户数据
        :param group_id: 群组ID
        :param win_user_id: 获胜的用户ID
        :param gold: 获得的金币数量
        :param fee: 手续费
        """
        user_id = str(user_id)
        ...
        if group_id not in self._player_data.keys():
            self._player_data[group_id] = {}
            logger.error("群组不在数据中")
        if user_id not in self._player_data[group_id].keys():
            logger.error("用户不在群组中")
            self._player_data[group_id][user_id] = {
                "user_id": user_id,
                "group_id": group_id,
                "nickname": nickname,
